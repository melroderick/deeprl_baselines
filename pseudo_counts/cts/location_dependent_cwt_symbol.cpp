...
location_dependent_cwt_symbol::location_dependent_cwt_symbol(int w, int h, int alphabet_size) {
    m_cwt_array = build_cwt_array(w, h, alphabet_size);
    m_width = w;
    m_height = h;
    m_alphabet_size = alphabet_size;
}
location_dependent_cwt_symbol::~location_dependent_cwt_symbol() {
    destroy_cwt_array(m_cwt_array, m_width, m_height);
}

double location_dependent_cwt_symbol::process_image(uint8 *image) {
...
uint8 location_dependent_cwt_symbol::get_pixel(uint8 *image, int x, int y, int w, int h) {
    if (x < w && x >= 0 && y < h && y >= 0) {
        return image[w*y + x];
    } else {
        return 0;
...
tuple<double, double> location_dependent_cwt_symbol::feed_bits_to_tree(cwt_symbol *tree, uint8 inp_pixel, uint8 *context) {
...
cwt_symbol ***location_dependent_cwt_symbol::build_cwt_array(int w, int h, int alphabet_size) {
    cwt_symbol ***cwt_array = new cwt_symbol**[w];
    int context_depth = 4;
    for (int x=0; x<w; x++) {
        cwt_array[x] = new cwt_symbol*[h];
        for (int y=0; y<h; y++) {
            cwt_array[x][y] = new cwt_symbol(context_depth, alphabet_size);
        }
    }
    return cwt_array;
}
...
void location_dependent_cwt_symbol::destroy_cwt_array(cwt_symbol ***cwt_array, int w, int h) {
    for (int x=0; x<w; x++) {
        for (int y=0; y<h; y++) {
...
void location_dependent_cwt_symbol::process_image_cols(cwt_symbol ***cwt_array, uint8 *image, tuple<double, double>* result, int from_x, int to_x, int w, int h) {
    double cum_prob = 0.0;
    double cum_recoding_prob = 0.0;

    for (int x=from_x; x < to_x; x++) {
        for (int y = 0; y < h; y++) {
            cwt_symbol *tree = cwt_array[x][y];
